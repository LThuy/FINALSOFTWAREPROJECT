    ...
    public partial class IncomingStockfrm : Form
    {
        ...
        private void comboBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            if(comboBox1.Text != "All")
            {
                ...
                String sSQL = "SELECT name,quantity,date,price,total_price FROM incoming_stock WHERE DATENAME(month, date) = '" + selectedMonth + "'";
            ...
            } else
            {
                SqlConnection conn = new SqlConnection(Program.strConn);
                conn.Open();
                String sSQL = "SELECT name,quantity,date,price,total_price FROM incoming_stock";
                SqlCommand cmd = new SqlCommand(sSQL, conn);
                SqlDataAdapter adapter = new SqlDataAdapter(cmd);
                DataTable dt = new DataTable();
                adapter.Fill(dt);
        ...
        private void btnFind_Click(object sender, EventArgs e)
        {
            SqlConnection conn = new SqlConnection(Program.strConn);
            conn.Open();
            String sSQL = "SELECT name,quantity,date,price,total_price FROM incoming_stock WHERE id LIKE '%" + txtID.Text + "%'";
            SqlCommand cmd = new SqlCommand(sSQL, conn);
            SqlDataAdapter adapter = new SqlDataAdapter(cmd);
            DataTable dt = new DataTable();
            adapter.Fill(dt);
